    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getStringExtra("isSlider").equals("true")) {
            ...
            if (userInput >= rightAnswer - (toleranceRange/100.0) * range && userInput <= rightAnswer + (toleranceRange/100.0) * range) {
                ...
                titleText = correct;
                answerVideo.setVideoURI(Uri.parse("android.resource://"+getPackageName()+"/"+R.raw.freuzirblrichtig)); //Videoquelle austauschen
                answerVideo.start();
                totalPoints.setText(String.format(Locale.GERMANY, "%d", scoreBefore));
            ...
            //Wenn AUSSERHALB des Toleranzbereichs des Slider, dann...
                ...
                titleText = wrong;
                answerVideo.setVideoURI(Uri.parse("android.resource://"+getPackageName()+"/"+R.raw.wrongzirbl)); //Videoquelle austauschen
                answerVideo.start();
                totalPoints.setText(String.format(Locale.GERMANY, "%d", scoreBefore));
            }
